...
- 只要任务涉及 `novels/*/正文/**/*.md` 的创作、续写、润色、改写，就必须默认采用人物当下可感知的叙述视角。
- 禁止把作者侧、提纲侧、编排侧的信息直接写进正文。尤其不要出现 `大纲里`、`卷纲里`、`设定里`、`本章`、`这章`、`前几章`、`上一章`、`下一章`、`这一卷`、`作者`、`读者`、`主角` 这类元叙事词，除非用户明确要求写吐槽、评论或章节外文本。
- 若一句话同时可以写成“提纲总结”或“人物体验”，必须改写为人物此刻能看到、听到、想到、回忆到、判断到的内容，优先落在感官、动作、现场信息和具体事件上。
- 若需要回收前文信息，不要写“前几章发生过什么”，要改成角色能成立的说法，例如 `这一路上`、`从白鸦渡到现在`、`他刚才听见的那句话`、`眼前这扇门`、`灰砾集那次悬赏`。
- 在完成正文修改前，必须主动检查一遍是否残留元叙事表述；若发现类似词语，先消除出戏感，再交付结果。
